...
from focalLoss import FocalLossTrainer #loss pondérée
from balancedFocalLoss import AdaptiveCBFLossTrainer #loss pondérée
...
def main():
    ...
    if labels_are_int:
        ...
        print("===================\n")
        #!!!!
    assert "IGN" not in label_to_id, "IGN ne devrait pas être dans le mapping"
    ...
    for batch in raw_datasets["train"]["ner_tags"]:
        for label in batch:
            if label != -100 and label != "IGN":  #!!!
                if isinstance(label, str):  # Si c'est un string comme 'B-T038'
                    all_labels.append(label_to_id[label])  # Utilisez le mapping
                else:  # Si c'est déjà un int
                    all_labels.append(label)

    # 2. Vérification des valeurs
    unique_labels = np.unique(all_labels)
    print(f"Labels uniques après filtrage: {unique_labels}")
    assert max(unique_labels) < num_labels, f"Label {max(unique_labels)} dépasse le nombre de classes ({num_labels})"

    # 3. Calcul des poids
    class_weights = torch.ones(num_labels, dtype=torch.float32)
    if len(unique_labels) > 0:  # Évite une erreur si pas de labels
        computed_weights = compute_class_weight(
            'balanced',
            classes=unique_labels,
            y=all_labels
        )
        for label_id, weight in zip(unique_labels, computed_weights):
            class_weights[label_id] = weight

    class_weights = class_weights.to(device)

    class_counts = torch.zeros(num_labels, dtype=torch.float32)
    for batch in raw_datasets["train"][label_column_name]:
        for label in batch:
            if label != -100 and label != "IGN":
                label_id = label if isinstance(label, int) else label_to_id[label]
    ...
    trainer = AdaptiveCBFLossTrainer(
        model=model,
        args=training_args,
