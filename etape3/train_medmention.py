...
def main():
    ...
    if config.model_type in {"bloom", "gpt2", "roberta", "deberta"}:
    ...
    else:#!!!! nouveau tokenizer qui ajoute un token de padding car neo n'en possède pas 
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            ...
            trust_remote_code=model_args.trust_remote_code,
            add_prefix_space=True,
            pad_token="<|pad|>"  # !!!!!Ajouter un token de padding car GPT-Neo n'en a pas par défaut
        )
        # vérification et ajout du pad_token si nécessaire
        if tokenizer.pad_token is None:
            tokenizer.add_special_tokens({'pad_token': '[PAD]'})  # Version plus standard
